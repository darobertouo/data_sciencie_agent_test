...
from .sub_agents import alloydb_agent, analytics_agent, bigquery_agent, social_listening_agent
...
async def call_social_listening_agent(
    question: str,
    tool_context: ToolContext,
):
    logger.debug("call_social_listening_agent: %s", question)

    # Se o agente BigQuery já trouxe dados, anexe ao prompt
    bigquery_data = tool_context.state.get("bigquery_query_result", "")
    question_with_context = f"""
    Pergunta: {question}
    
    Dados disponíveis do BigQuery:
    {bigquery_data}
    """

    agent_tool = AgentTool(agent=social_listening_agent)

    output = await agent_tool.run_async(
        args={"request": question_with_context}, tool_context=tool_context
    )

    tool_context.state["social_listening_output"] = output
    return output
